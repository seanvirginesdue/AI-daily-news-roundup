...
type Recipient = { first_name: string; email: string };

export default function RecipientsPage() {
  const [recipients, setRecipients] = useState<Recipient[]>([]);
  const [editing, setEditing]       = useState<number | null>(null);
  const [draft, setDraft]           = useState<Recipient>({ first_name: "", email: "" });
  const [adding, setAdding]         = useState(false);
  const [newR, setNewR]             = useState<Recipient>({ first_name: "", email: "" });
  ...
  const del = async (i: number) => {
    ...
    await fetch(`/api/recipients/${i}`, { method: "DELETE" });
    load();
  };

  const add = async () => {
    if (!newR.first_name || !newR.email) return;
    await fetch("/api/recipients", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify(newR),
    });
    setAdding(false); setNewR({ first_name: "", email: "" }); load();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{recipients.length} recipients configured</p>
        </div>
            ...
            {editing === i ? (
              <div className="flex gap-2 items-center">
                <input value={draft.first_name} onChange={e => setDraft({ ...draft, first_name: e.target.value })}
                  placeholder="First name" className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-36 focus:outline-none focus:ring-2 focus:ring-[#d63c2f]" />
                <input value={draft.email} onChange={e => setDraft({ ...draft, email: e.target.value })}
                  placeholder="Email" className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm flex-1 focus:outline-none focus:ring-2 focus:ring-[#d63c2f]" />
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="font-medium text-sm text-gray-900">{r.first_name}</p>
                  <p className="text-xs text-gray-400 truncate mt-0.5">{r.email}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => { setEditing(i); setDraft(r); }}
      ...
      {adding && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
            <h2 className="font-bold text-lg mb-4">Add Recipient</h2>
            <label className="block text-sm font-medium text-gray-700 mb-1">First Name</label>
            <input value={newR.first_name} onChange={e => setNewR({ ...newR, first_name: e.target.value })}
            ...
            <input value={newR.email} onChange={e => setNewR({ ...newR, email: e.target.value })}
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={add} className="bg-[#d63c2f] text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-[#b83326]">Add Recipient</button>
            </div>
          </div>
        </div>
      )}
